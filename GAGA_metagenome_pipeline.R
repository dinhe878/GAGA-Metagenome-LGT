...
#GC_medianCI.euk <- wilcox.test(chrSum.euk$GC, alternative="two.sided", conf.int=T, conf.level=0.95)
GC_medianCI.euk <- MedianCI(chrSum.euk$GC, method = "boot", conf.level = 0.95)
#Coverage_medianCI.euk <- wilcox.test(chrSum.euk$coverage, alternative="two.sided", conf.int=T, conf.level=0.95)
...
# save the chrSum table
write.table(chrSum,file=paste(folder,"chrSum.csv",sep=""),quote=F,sep="\t",row.names=F)
#qplot(chrSum$euk.wind.perc)
...
if (length(type.pro.df[,]) > 0) {
  ...
  top_type.pro.df <- if (length(type.pro.df.ordered[,1]) >= 10) type.pro.df.ordered[1:10,] else type.pro.df.ordered[,]
  top_type.pro.char <- as.character(top_type.pro.df$Var1)
  top_type.pro.char <- append(top_type.pro.char, "other pro sp.")
  top_fewer_type.pro.df  <- if (length(type.pro.df.ordered[,1]) >= 5) type.pro.df.ordered[1:5,] else type.pro.df.ordered[,]
  top_fewer_type.pro.char <- as.character(top_fewer_type.pro.df$Var1)
