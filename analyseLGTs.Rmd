...
lgt.candidate.bam<-readGAlignments(paste(filepath,"LGTs.5kb.candidateregions.PacBio.bam",sep=""))
...
colnames(bh)[1:5]<-c("window","probit","protax","eukbit","euktax")

# set NA bitscores to 0
...
if(nrow(lgt)<30){
  pc.overview<-plot_grid(pcplot+theme(legend.position = "none"),overviews,ncol=1,rel_heights = c(3/5,2/5))
}else{
  pc.overview<-plot_grid(pcplot+theme(legend.position = "none"),overviews,ncol=1,rel_heights = c(30,nrow(lgt)))
}
overviewFinal<-plot_grid(pc.overview,as_ggplot(leg),ncol=2,rel_widths = c(0.8,0.2))

ggsave(filename = paste(filepath,GAGAid,".lgt.candidates.pdf",sep=""),width = 10, height=ifelse(nrow(lgt)>30,nrow(lgt)/6,10),limitsize = F)
...
allPlots<-foreach(selection=1:length(lgt.stackedL)) %dopar% {
  ...
  seqP<-ggmsa(lgt.candidate.fasta.subset,1,ifelse(width(lgt.candidate.fasta.subset)>75,75,lgt.candidate.fasta.subset),color="Shapely_NT")+theme_nothing()+  scale_x_continuous(expand=c(0,0)) + scale_y_continuous(expand=c(0,0)) + labs(x = NULL, y = NULL)
  
  
  ## Bam processing
  ### plot for entire region and for single candidate
  lgt.locus.range<-unlist(strsplit(lgt.stackedL[[selection]]$locus[1],":"))
  ...
  blsPlot<-ggplot()+
          ...
          geom_text(data=lgt.candidates.subset,y=0.15,aes(x=cand.start[1]-100),label=paste(BestBlastnHit[8]," (",BestBlastnHit[1],")\ne-value = ",BestBlastnHit[4],sep=""),hjust=0,size=3)+
  ...
  plotbh<-ggplot(sbh)+
        geom_step(aes(x=windowstart,y=log(eukbit,10)),col="lightblue",size=1.5)+
        geom_step(aes(x=windowstart,y=log(probit,10)),col="orange",size=1.5)+
        coord_cartesian(xlim=c(min(lgt.stackedL[[selection]]$covWindowCenter),max(lgt.stackedL[[selection]]$covWindowCenter)))+
        annotate("text",  x=Inf, y = Inf, label = c("euk","pro"),col=c("lightblue","orange"), vjust=1, hjust=c(1,2.1),size=2)+xlab(sbh$scaffold[1])+
        geom_segment(data=subset(lgt.candidates,cand.scaffold==lgt.candidates.subset$cand.scaffold),aes(x=cand.start,xend=cand.end),y=0,yend=0,col="red",size=1,alpha=.5)+
        geom_rect(data=lgt.candidates.subset,aes(xmin=cand.start,xmax=cand.end),ymin=0,ymax=-Inf,fill="red3",size=1,alpha=1)+
  ...
  if(length(lgt.bam.select) > 0){
    p4<-autoplot(lgt.bam.select,aes(fill=strand),color=rgb(0,0,0,0))+guides(fill="none")+
      ...
      geom_rect(data=lgt.candidates.subset,aes(xmin=cand.start,xmax=cand.end),ymin=0,ymax=-Inf,fill="red3",size=1,alpha=1)+
      geom_rect(data=lgt.candidates.subset,aes(xmin=cand.start,xmax=cand.end),ymin=0,ymax=Inf,fill="red",size=1,alpha=.1)+
      coord_cartesian(xlim=c(min(lgt.stackedL[[selection]]$covWindowCenter),max(lgt.stackedL[[selection]]$covWindowCenter)),ylim=c(0,200))+
      theme_light()+
      theme(axis.text = element_text(size=8))+
      xlab(NULL)
    #reorder layers
...
## make composite plot  
regionPlot<-  plot_grid(alignmentPlotplot,p1,p3,plotbh,p4plot,rel_heights = c(0.2,0.1,0.001,0.1,0.3),ncol=1,align = "v")
candidatePlot<-plot_grid(p5plot,blsPlot,seqP,rel_heights = c(0.5,0.49,0.01),ncol=1,align = "v")

tmpPlot<-plot_grid(regionPlot,candidatePlot,rel_heights=c(0.5,0.5),ncol=2)
finalPlot<-plot_grid(scfPlot,tmpPlot,rel_heights=c(0.03,0.97),ncol=1)
}
```
...
for (i in 1:length(allPlots)){
  ggsave(paste(filepath,"lgt.candidates/",GAGAid,".",gsub(":","-",names(lgt.stackedL)[i]),".pdf",sep=""),allPlots[[i]],width=16,height=12)
}
```
