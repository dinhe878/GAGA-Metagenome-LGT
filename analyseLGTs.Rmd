...
allPlots<-foreach(selection=1:length(lgt.stackedL)) %dopar% {
  ...
  blsPlot<-ggplot()+
          ...
          geom_text(data=lgt.candidates.subset,y=0.15,aes(x=lgt.candidates.subset$cand.start[1]-100),label=paste(BestBlastnHit[8]," (",BestBlastnHit[1],")\ne-value = ",BestBlastnHit[4],sep=""),hjust=0)+
  ...
  if(length(lgt.bam.select) > 0){
    p4<-autoplot(lgt.bam.select,aes(fill=strand),color=rgb(0,0,0,0))+guides(fill="none")+
      ...
      xlab(NULL)
    #reorder layers
    p4@ggplot$layers<-p4@ggplot$layers[c(4,1,2,3)]
    p4plot<-p4@ggplot
  }
...
## make composite plot  
tmpPlot<-plot_grid(blsPlot,alignmentPlotplot,p1,p3,p4plot,p5plot,seqP,ncol=1,align = "v",rel_heights = c(0.3,0.2,0.1,0.001,0.3,0.3,0.05))
