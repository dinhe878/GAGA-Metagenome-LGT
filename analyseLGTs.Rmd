...
pc.overview<-plot_grid(pcplot+theme(legend.position = "none"),overviews,ncol=1,rel_heights = ifelse(nrow(lgt)>30,c((1-(nrow(lgt)/10)),nrow(lgt)/10),c(3/5,2/5)))
overviewFinal<-plot_grid(pc.overview,as_ggplot(leg),ncol=2,rel_widths = c(0.8,0.2))

ggsave(filename = paste(filepath,GAGAid,".lgt.candidates.pdf",sep=""),width = 10, height=ifelse(nrow(lgt)>30,nrow(lgt)/3,10))
```
...
allPlots<-foreach(selection=1:length(lgt.stackedL)) %dopar% {
  ...
  p2<-ggplot(lgt.stackedL[[selection]],aes(x=covWindowCenter,y=cov))+
    ...
    geom_segment(data=lgt.candidates,aes(x=cand.start,xend=cand.end),y=0,yend=0,col="red",size=1,alpha=.5)+
