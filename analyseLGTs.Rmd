...
ggsave(filename = paste(filepath,GAGAid,".lgt.candidates.pdf",sep=""),width = 10, height=ifelse(nrow(lgt)>40,nrow(lgt)/6,10),limitsize = F)
...
allPlots <- foreach(selection=1:length(lgt.stackedL)) %dopar% {
  ...
  seqP <- ggmsa(lgt.candidate.fasta.subset,1,ifelse(width(lgt.candidate.fasta.subset)>75,75,lgt.candidate.fasta.subset),color="Shapely_NT")+
          theme_nothing() + 
  ...
  p2 <-     ggplot(lgt.stackedL[[selection]],aes(x=covWindowCenter,y=cov))+
            ...
            coord_cartesian(xlim=c(min(lgt.stackedL[[selection]]$covWindowCenter),
                                   max(lgt.stackedL[[selection]]$covWindowCenter)))+
            theme_light()+
            theme(axis.text = element_text(size=8))+
            xlab(NULL)
