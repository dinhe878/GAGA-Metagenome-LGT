...
lgt.complexity<-read.csv("/Users/lukas/sciebo/Projects/LGT/results/GAGA-0515/LGTs.candidateloci.loose.complex",sep="\t")
...
pcplot<-ggplot(lgt.pca)+geom_point(aes(x=PC1,y=PC2,color=locus,size=log(abs(cand.start-cand.end),10)))+
  ...
  ylab(paste("PC1 (",round(summary(pca1)$importance[2,2]*100,2),"%)",sep=""))+
...
pc.overview<-plot_grid(pcplot+theme(legend.position = "none"),overviews,ncol=1,rel_heights = c(2/5,3/5))
overviewFinal<-plot_grid(pc.overview,as_ggplot(leg),ncol=2,rel_widths = c(0.8,0.2))

ggsave(filename = paste(filepath,GAGAid,".lgt.candidates.pdf",sep=""),width = 10, height=nrow(lgt)/3)
```
